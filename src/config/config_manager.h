...
protected:
    ...
    std::shared_ptr<ConfigOption> setOption(pugi::xml_node root, ConfigVal option, const std::map<std::string, std::string>* arguments = nullptr);
    /// @brief cache all nodes from config file
    void getAllNodes(pugi::xml_node parent, bool getAttributes = true);
